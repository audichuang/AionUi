...
export const fs = {
  ...
  detectAndCountExternalSkills: bridge.buildProvider<IBridgeResponse<Array<{ name: string; path: string; source: string; skills: Array<{ name: string; description: string; path: string }> }>>, void>('detect-and-count-external-skills'),
  // 符号链接方式导入 skill / Import skill via symlink
  importSkillWithSymlink: bridge.buildProvider<IBridgeResponse<{ skillName: string }>, { skillPath: string }>('import-skill-with-symlink'),
  ...
  exportSkillWithSymlink: bridge.buildProvider<IBridgeResponse, { skillPath: string; targetDir: string }>('export-skill-with-symlink'),
  // 自定义外部技能路径管理 / Custom external skill paths management
  getCustomExternalPaths: bridge.buildProvider<Array<{ name: string; path: string }>, void>('get-custom-external-paths'),
  addCustomExternalPath: bridge.buildProvider<IBridgeResponse, { name: string; path: string }>('add-custom-external-path'),
